...
tick, on start/stop, after a spawn, and after `message.deliverNow` / `actor.invoke` /
`actor.revive`, since actor code may change state outside a tick.
...
| Command | Payload fields | Result | Error codes |
|---|---|---|---|
| `message.send` | `from: int`, `to: int`, `payload: any` | null | `ACTOR_NOT_FOUND`, `ACTOR_DEAD` (sender) |
...
| `actor.invoke` | `actorId: int`, `method: string`, `args: [any, ...]` | method return value | `ACTOR_NOT_FOUND`, `ACTOR_DEAD`, `METHOD_NOT_FOUND`, `INVALID_ARGUMENT` |
| `actor.kill` | `actorId: int` | null | `ACTOR_NOT_FOUND` |
| `actor.revive` | `actorId: int` | null | `ACTOR_NOT_FOUND` |
...
A killed actor keeps its state (and `actor.setField` still works on it), but messages arriving at
it are dropped with a `message.dropped` event carrying a `reason`, it can't send, and it can't be
invoked. Messages it sent before being killed are still delivered. On `actor.revive`, the actor's
`OnRevive()` is called if it implements the `Reviver` interface. Killing a dead actor and reviving
a live one are no-ops that emit no event.
...
| `message.dropped` | `messageId`, `reason` (only when the simulator dropped it, e.g. the recipient is dead) |
| `message.delayed` | `messageId`, `newDeliverTick` |
| `actor.spawned` | `actorId`, `typeName` |
| `actor.fieldChanged` | `actorId`, `field`, `value` |
| `actor.killed` | `actorId` |
| `actor.revived` | `actorId` |
